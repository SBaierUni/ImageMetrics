...
if (len(sys.argv) < 2) | (int(sys.argv[1]) > len(format_ratio)):
		print("Usage: python autorun.py [pick ...]\n\n"
		 "Available picks:\n1\tjp2_10\n2\tjp2_30\n3\tjpg_10\n4\tjpg_30\n")
		exit()
			...
			for cr in com_ratios:
				...
				for m in metrics:
					...
					if [f, cr, m] not in dat:
						dat.append([f, cr, m])
						df = df.append({"format" : str(f) + "_" + str(cr), "metric" : m, "avg" : avg / float(len(filename))}, ignore_index=True)
		outputs.append([sf, sr, df])

plt.figure()
sf, sr, df = outputs[format_ratio_pick]
plt.title(sf + "_" + str(sr))
